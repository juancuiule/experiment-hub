...
| `invalid-split-size`    | A split formula has `size`/`into` ≤ 0 or not an integer                                   |
| `split-bins-exceed-items` | A split formula's `into` exceeds the length of an inline (static) input list            |
